...
{
   if(snpName.Length()==0 || chromosomes.Length()==0 || positions.Length()==0){
         ...
         for(int m = 0; m < mMax; m++){
            ...
            for(int t = 0; t < N_Trait; t++){// computationally intensive for large N_Trait
               ...
               fprintf(fps[thread], "%s\t%s\t%d\t%d\t%s\t%s\t%.3lf\t%d\t%.4lf\t%.4lf\t%.4lf\t%.4lf\t%.3G\n",
                  ...
                  chromosomes[pos], int(positions[pos]*1000000+0.5),
   ...
   for(int blockb = 0; blockb < shortCount; blockb += CACHESIZE){
      ...
      if(chisqFilter!=_NAN_){  // filter P-value, faster computation
         for(int m = 0; m < mMax; m++){
            ...
            /*
            if(maxT == -1) continue;   // no significant trait

            lxy = -Sum_VR[maxT];
            for(int i = 0; i < GM[m][0]; i++)
               lxy += VR[maxT][GM[m][i+1]];
            lxy *= freq[m];
            for(int i = 0; i < G2[m][0]; i++)
               lxy += VR[maxT][G2[m][i+1]];
            lxy += lxy;
            for(int i = 0; i < G1[m][0]; i++)
               lxy += VR[maxT][G1[m][i+1]];

            RR = RA = 0.0;
            for(int i = 0; i < N_ID; i++){
               RR += VR[maxT][i] * VR[maxT][i];
               RA += VR[maxT][i];
            }
            RA /= N_ID;
            lxx = RR - RA*RA*N_ID;
            ...
            fprintf(ft[thread], "%d\t%s\t0\t%d",
               chromosomes[pos], (const char*)snpName[pos], int(positions[pos]*1000000 + 0.5));
            for(int i = 0; i < N_ID; i++){
               vn = v0 + VR[maxT][i] * v1;
               if(vn <= 0.5)
                  fprintf(ft[thread], "\t%s\t%s",
               (const char*)alleleLabel[1][pos], (const char*)alleleLabel[1][pos]);
               else if(vn >= 1.5)
                  fprintf(ft[thread], "\t%s\t%s",
               (const char*)alleleLabel[0][pos], (const char*)alleleLabel[0][pos]);
               else
                  fprintf(ft[thread], "\t%s\t%s",
...
//      fclose(ft[thread]);
   ...
   if(chisqFilter!=_NAN_){
      ft[0] = fopen(tfile, "at");
      for(int c = 1; c < CoreInUse; c++){
         ft[c] = fopen(tfiles[c], "rt");
         while(fgets(buffer, 60000, ft[c]) != NULL){
            fputs(buffer, ft[0]);
         }
         fclose(ft[c]);
         remove(tfiles[c]);
...
#endif
/*
   String tfamfile(prefix);
   if(chisqFilter!=_NAN_){
      tfamfile.Add(".tfam");
      ft[0] = fopen((const char*)tfamfile, "wt");
      for(int i = 0; i < N_ID; i++)
         fprintf(ft[0], "%s%s\t%s\t%s\t%s\t%d\t%d\n",
            (const char*)prefix, (const char*)ped[ID[i]].famid,
            (const char*)ped[ID[i]].pid,
            (const char*)ped[ID[i]].fatid,
            (const char*)ped[ID[i]].motid,
            ped[ID[i]].sex,
            ped[ID[i]].affections[0]);
   }
*/
   fclose(fps[0]);
//   if(chisqFilter!=_NAN_)
...
/*
   if(chisqFilter!=_NAN_){
      printf("Predicted genotypes saved in files %s and %s\n",
         (const char*)tfile, (const char*)tfamfile);
      printf("Note: run following commands to compare trait-predicted vs. true genotypes:\n");
      printf("     plink2 --tfile %s --make-bed --out %s_pred\n",
         (const char*)prefix, (const char*)prefix);
      printf("     king -b %s,%s_pred --kinship --prefix %s\n\n",
         (const  char*)plinkinput, (const char*)prefix, (const char*)prefix);
   }
*/
   for(int t = 0; t < N_Trait; t++)
      delete VR[t];
...
//         for(int m = 0; m < mMax; m++)
//            for(int t = 0; t < N_Trait; t++)
//               scores_local[m][t] *= freq[m];
//         for(int m = 0; m < mMax; m++)
//            for(int t = 0; t < N_Trait; t++)
//               scores_local[m][t] *= 2.0;
...
/*
   ...
   */
   /*
   if(permuteCount){
      fprintf(fp, "\tLabelA\tLabela\tChisq\tP_unadj\tP_adj");
      if(N_Trait > 1) fprintf(fp, "\tP_adj2");
      fprintf(fp, "\n");
      GenomeScanWithPermutation(fp);
   }else{
      if(dosagefile==""){
//         fprintf(fp, "\tLabelA\tLabela\tFreqA\tN\tBeta\tSE\tChisq\tH2\tPvalue\n");
         GenomeScan();   // genome scan
      }else{
         fprintf(fp, "\tLabelA\tLabela\tFreqA\tInfo\tN\tBeta\tSE\tChisq\tH2\tPvalue\n");
         DosageScan(fp);
      }
   }
//   fclose(fp);
*/
//   printf("Score-MT genome scan results saved in file %s\n\n", (const char*)lmmfile);
//   if(chisqFilter==_NAN_ && (!permuteCount) && (dosagefile==""))
//      PostVC();
...
//   D.Multiply(-0.5);
   ...
   Matrix UR(N_ID, N_Trait);
   UR.Zero();
   for(int i = 0; i < N_ID; i++)
      for(int t = 0; t < N_Trait; t++)
         for(int k = 0; k < N_ID; k++)
            UR[i][t] += UT[i][k] * Resid0[k][t];
   ...
   for(int t = 0; t < N_Trait; t++){
      ...
      for(int i = 0; i < N_ID; i++)
         for(int j = 0; j < N_ID; j++)
            VR[t][i] += UT[j][i] * UR.data[j]->data[t] / (lambda0[t]*EV[j]+1) * tau0[t];
      for(int i = 0; i < N_ID; i++)
         varVR[t] += UR[i][t] * UR[i][t] * EV[i] * tau0[t] * tau0[t] / (lambda0[t]*EV[i]+1) / (lambda0[t]*EV[i]+1);
      for(int i = 0; i < N_ID; i++)
         VR[t][i] /= sqrt(varVR[t]);
   }
   for(int i = 0; i < N_ID; i++)
   ...
   */
//      for(rank = permuteCount-1; rank >=0 && maxStat[rank] > chisq[m]; rank--);
      /*
      if(chisq[m] > maxStat[permuteCount-1]) pvalue = 0.5 / permuteCount;
      else if(chisq[m] < maxStat[0]) pvalue = 1-0.5 / permuteCount;
      else{
         left = 0;
         right = permuteCount-1;
         middle = (left+right)/2;
         for(; left < right-1; middle=(left+right)/2){
            if(maxStat[middle] > chisq[m]) right = middle;
            else if(maxStat[middle] < chisq[m]) left = middle;
            else {
               rank = middle;
               break;
            }
...
/*
void Engine::GenomeScan(FILE *fp)
{
   int N_ID = ID.Length();
   int N_Trait = traits.Length();
   double pvalue, h2, freq[16], statistic, score_local, var_snp, Tm;
   int missingCount[16], id, AA, Aa, missing, pos;
   if(chisqFilter==_NAN_){
      chisqs = new Vector[N_Trait];
      for(int t = 0; t < N_Trait; t++)
         chisqs[t].Dimension(0);
   }
   int *G2[16], *G1[16], *GM[16], *B2[16], *B1[16], *BM[16];
   for(int m = 0; m < 16; m++){
      G2[m] = new int[N_ID+1];
      G1[m] = new int[N_ID+1];
      GM[m] = new int[N_ID+1];
   ...
   Sum_Block = new double **[N_Trait];
   for(int t = 0; t < N_Trait; t++){
      Sum_Block[t] = new double *[N_Block];
      for(int i = 0; i < N_Block; i++)
         Sum_Block[t][i] = new double[256];
   }
   for(int t = 0; t < N_Trait; t++)
      for(int b = 0; b < N_Block; b++)
         for(int i = 0; i < 256; i++){
            ...
            for(int k = 0; k < 8; k++)
               if(itov[i][k])
                  Sum_Block[t][b][i] += VR[t][b*8+k];
         }
   double *Sum_VR = new double[N_Trait];
   for(int t = 0; t < N_Trait; t++){
      Sum_VR[t] = 0;
      for(int b = 0; b < N_Block; b++)
         Sum_VR[t] += Sum_Block[t][b][255];
   }
   int bNum, bPos;
   for(int b = 0; b < shortCount; b++){ // loop over short integer
      for(int m = 0; m < 16; m++){
      ...
      for(int m = 0; m < 16; m++){
         ...
         for(int t = 0; t < N_Trait; t++){// computationally intensive for large N_Trait
            ...
            for(int i = 0; i < GM[m][0]; i++){
               bNum = GM[m][i+1];
               score_local += Sum_Block[t][bNum][BM[m][bNum]];
            }
            score_local *= freq[m];
            for(int i = 0; i < G2[m][0]; i++){
               bNum = G2[m][i+1];
               score_local += Sum_Block[t][bNum][B2[m][bNum]];
            }
            score_local += score_local;
            for(int i = 0; i < G1[m][0]; i++){
               bNum = G1[m][i+1];
               score_local += Sum_Block[t][bNum][B1[m][bNum]];
            }
            if((score_local > -Tm) && (score_local < Tm)) continue;
            statistic = score_local * score_local / var_snp;
            pvalue = ndist(sqrt(statistic))*2;
            h2 = score_local * score_local / varVR[t] / var_snp;
            h2 = h2/((1+lambda0[t])/tau0[t]+h2);
            ...
            if(positions.Length())
               fprintf(fp, "\t%.6lf", positions[pos]);
            fprintf(fp, "\t%c\t%c\t%.3lf\t%d",
               alleleLabel[0][pos], alleleLabel[1][pos], freq[m], N_ID-missingCount[m]);
            if(statistic != _NAN_)
               fprintf(fp, "\t%.4lf\t%.4lf\t%.4lf\t%.4lf\t%.2G\n",
                  score_local / sqrt(varVR[t]) / var_snp,
                  1/sqrt(varVR[t]*var_snp),
                  statistic, h2, pvalue);
            else
  ...
  */
//         for(int i = 0; i < N_ID; i++)
//            score_local += dosage[i] * VR[t][i];
      /*
         freq = 0;
         for(int i = 0; i < valid[t][0]; i++)
            freq += dosage[valid[t][i+1]];
         freq /= (valid[t][0]*2);
         if(freq < 1E-10 || freq > 1-1E-10) continue;
         var_snp = 0;
         for(int i = 0; i < valid[t][0]; i++){
            pos = valid[t][i+1];
            var_snp += dosage[pos] * dosage[pos];
      ...
      freq /= ((N_ID-missingCount)*2);
      if(missingCount)
      for(int i = 0; i < N_ID; i++)
         if(key[i]==-1)
            dosage[i] = freq*2;
      var_snp = 2*freq*(1-freq);
      if(var_snp < 1E-10) continue;
      Tm = sqrt(chisqFilter*var_snp);
      */
...
#ifdef _OPENMP
   int CoreInUse=CoreCount?CoreCount:defaultMaxCoreCount;
   printf("Scanning autosomes with %d CPU cores...\n", CoreInUse);
   fps = new FILE *[CoreInUse];
   StringArray outfiles(CoreInUse);
   for(int c = 1; c < CoreInUse; c++){
      outfiles[c].Copy(prefix);
      outfiles[c] += (c+1);
      outfiles[c].Add("$$$.mtscore");
...
#ifdef _OPENMP
   #pragma omp parallel num_threads(CoreInUse) \
   private(freq, missingCount, pos, var_snp, Tm, id, AA, Aa, missing, \
   score_local, pvalue, statistic, h2, thread, v0,v1,vn, RR, RA, maxT, maxR, lxy, lxx)
{
#endif
   int *G2[16], *G1[16], *GM[16];
   for(int m = 0; m < 16; m++){
