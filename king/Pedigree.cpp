         ...
         if  (!persons[i]->isFounder() && persons[i]->sibs[0] == persons[i])
            ...
            for (int j = 0; j < persons[i]->sibCount; j++)
               if (persons[i]->sibs[j]->isGenotyped(m))
                  ...
                  int fat1 = fat.hasAllele(geno[1]);
                  int fat2 = fat.hasAllele(geno[2]);
                  int mot1 = mot.hasAllele(geno[1]);
                  int mot2 = mot.hasAllele(geno[2]);
                  ...
                  if (fgeno && mgeno && !(fat1 && mot2 || fat2 && mot1) ||
                     ...
                     printf("%s - Fam %s: Child %s [%s/%s] has ",
                         ...
                         (const char *) info->GetAlleleLabel(geno[1]),
                         (const char *) info->GetAlleleLabel(geno[2]));

                     if (!fgeno || !mgeno)
                        printf("%s [%s/%s]\n",
                          fgeno ? "father" : "mother",
                         (const char *) info->GetAlleleLabel(fgeno ? fat[1] : mot[1]),
                         (const char *) info->GetAlleleLabel(fgeno ? fat[2] : mot[2]));
                     else
                        printf("parents [%s/%s]*[%s/%s]\n",
                               (const char *) info->GetAlleleLabel(fat[1]),
                               (const char *) info->GetAlleleLabel(fat[2]),
                               (const char *) info->GetAlleleLabel(mot[1]),
                               (const char *) info->GetAlleleLabel(mot[2]));
                     ...
                     {
                     if (haplos[geno[1]] != i) { haplo++; haplos[geno[1]] = i;};
                     if (haplos[geno[2]] != i) { haplo++; haplos[geno[2]] = i;};
                     ...
                     if (genos[index] != i)
                        ...
                        if (geno.isHomozygous())
                           ...
                           counts[geno[2]]++;
                        if (counts[geno[1]] > 2) too_many_genos = true;
                        if (counts[geno[2]] > 2) too_many_genos = true;
                        }
                     }
                  }
            ...
            if (fgeno)
               {
               if (haplos[fat[1]] != i) { haplo++; haplos[fat[1]] = i; }
               if (haplos[fat[2]] != i) { haplo++; haplos[fat[2]] = i; }
               homo += fat.isHomozygous();
               }

            if (mgeno)
               {
               if (haplos[mot[1]] != i) { haplo++; haplos[mot[1]] = i; }
               if (haplos[mot[2]] != i) { haplo++; haplos[mot[2]] = i; }
            ...
            if (diplo > 4 || haplo + homo > 4 || haplo == 4 && too_many_genos )
               ...
               for (int j = 0; j < persons[i]->sibCount; j++)
                  printf("%s%s/%s", j == 0 ? "" : " ",
                  (const char *) info->GetAlleleLabel(persons[i]->sibs[j]->markers[m][1]),
                  (const char *) info->GetAlleleLabel(persons[i]->sibs[j]->markers[m][2]));
               printf("]\n");
         ...
         for (int i = families[f]->first; i <= families[f]->last; i++)
            if (persons[i]->sex == SEX_MALE && persons[i]->markers[m].isKnown() &&
               ...
               printf("%s - Fam %s: Male %s has two X alleles [%s/%s]\n",
                  ...
                  (const char *) info->GetAlleleLabel(persons[i]->markers[m][1]),
                  (const char *) info->GetAlleleLabel(persons[i]->markers[m][2]));

               // Wipe this genotype so we don't get cascading errors below
               //persons[i]->markers[m][0] = persons[i]->markers[m][1] = 0;
               persons[i]->markers[m].Wipe();
         ...
         if  (!persons[i]->isFounder() && persons[i]->sibs[0] == persons[i])
            ...
            for (int j = 0; j < persons[i]->sibCount; j++)
               if (persons[i]->sibs[j]->isGenotyped(m))
                  ...
                  bool fat1 = fat.hasAllele(geno[1]);
                  bool fat2 = fat.hasAllele(geno[2]);
                  bool mot1 = mot.hasAllele(geno[1]);
                  bool mot2 = mot.hasAllele(geno[2]);
                  ...
                  if (sex == SEX_MALE)
                     ...
                     if (mgeno && !mot1)
                        ...
                        mother_ok &= inferred_mother.AddAllele(geno[1]);
                     }
                  if (sex == SEX_FEMALE)
                     {
                     if (fgeno && mgeno && !(fat1 && mot2 || fat2 && mot1) ||
                        ...
                        if (!sisters++)
                           ...
                           mother_ok &= inferred_mother.AddAllele(
                                 geno.otherAllele(inferred_father[1]));
                           mother_ok &= inferred_mother.AddAllele(
                                 first_sister.otherAllele(inferred_father[1]));
                           }

                        if (!fgeno && (mot1 ^ mot2))
                           inferred_father.Intersect(mot1 ? geno[2] : geno[1]);

                        if (!mgeno && (fat1 ^ fat2))
                           mother_ok &= inferred_mother.AddAllele(fat1 ? geno[2] : geno[1]);
                        }
                     }
                  }
