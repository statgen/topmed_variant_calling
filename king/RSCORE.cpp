...
*/
   /*
   int *G2 = new int[N_ID+1];
   int *G1 = new int[N_ID+1];
   int *GM = new int[N_ID+1];
   ...
   int pos;
   int k, mm;
   double statistic, score_local, Tm;
   for(int b = 0; b < shortCount; b++){ // loop over short integer
      for(int i = 0; i < N_ID; i++){
         id = geno[ID[i]];
         AA = GG[0][id][b] & GG[1][id][b];
      ...
      for(int m = 0; m < 16; m++){
         ...
         double var_snp = 2*freq[m]*(1-freq[m]);
         if(var_snp < 1E-10) continue;
         G2[0] = G1[0] = GM[0] = 0;
         for(int i = 0; i < N_ID; i++)
            if(dataG[i][m] == 2){
         ...
         for(int t = 0; t < N_Trait; t++){
            score_local = 0;
            for(int i = 0; i < GM[0]; i++)
               score_local += VR.data[t]->data[GM[i+1]];
            score_local *= freq[m];
            for(int i = 0; i < G2[0]; i++)
               score_local += VR.data[t]->data[G2[i+1]];
            score_local += score_local;
            for(int i = 0; i < G1[0]; i++)
               score_local += VR.data[t]->data[G1[i+1]];
            score_local -= 2*freq[m]*Sum_VR.data[t];
            if((chisqFilter!=_NAN_) && (score_local > -Tm) && (score_local < Tm))
               continue;      // for Pvalue filter
            statistic = score_local * score_local / var_snp;
            pvalue = ndist(sqrt(statistic))*2;
            h2 = score_local * score_local / var[t] / var_snp;
            h2 = h2/((1+lambda0[t])/tau0[t]+h2);
            chisqs[t].Push(statistic);
            if(snpName.Length())
               fprintf(fp, "%s", (const char*)snpName[pos]);
            ...
            if(statistic != _NAN_)
               fprintf(fp, "\t%.4lf\t%.4lf\t%.4lf\t%.4lf\t%.2G\n",
                  score_local / sqrt(var[t]) / var_snp,
                  1 / sqrt(var[t]*var_snp),
                  statistic, h2, pvalue);
            else
               fprintf(fp, "\t%s\t%s\n", "NA", "NA");
         }
...
/*
   ...
   for(int t = 0; t < N_Trait; t++){
      ...
      for(int i = 0; i < N_ID; i++)
         for(int j = 0; j < N_ID; j++)
            VR[t][i] += UT[j][i] * Resid0[j][t] * tau0[t] / (lambda0[t]*EV[j]+1);
      for(int i = 0; i < N_ID; i++)
         varVR[t] += Resid0[i][t] * Resid0[i][t] * EV[i] * tau0[t] * tau0[t] / (lambda0[t]*EV[i]+1) / (lambda0[t]*EV[i]+1);
      for(int i = 0; i < N_ID; i++)
         VR[t][i] /= sqrt(varVR[t]);
   }
   for(int i = 0; i < N_ID; i++)
...
//         if(var_snp < 1E-10) continue;
//         Tm = sqrt(chisqFilter*var_snp);
//         if(freq0[m] < 0.1 || freq0[m]>N_ID-missingCount0[m]-0.1) continue;
         for(int t = 0; t < N_Trait; t++){// computationally intensive for large N_Trait
            ...
            if(positions.Length())
               fprintf(fp, "\t%.6lf", positions[pos]);
            fprintf(fp, "\t%c\t%c\t%.3lf\t%d",
               alleleLabel[0][pos], alleleLabel[1][pos], freq, N_ID-missingCount);
            if(statistic != _NAN_)
               fprintf(fp, "\t%.4lf\t%.4lf\t%.4lf\t%.4lf\t%.2G\n",
                  score_local / sqrt(varVR[t]) / var_snp,
                  1/sqrt(varVR[t]*var_snp),
