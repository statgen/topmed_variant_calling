...
void Matrix::Copy(const Matrix & m)
   ...
   if (m.data != NULL)
      for (int i = 0; i < rows; i++)
         for (int j = 0; j < cols; j++)
            (*this).data[i]->data[j] = m.data[i]->data[j];
   }
...
void Matrix::Product(const Matrix & l, const Matrix & r)
   ...
   for (int k = 0; k < l.cols; k++)
      for (int i = 0; i < rows; i++)
         for (int j = 0; j < cols; j++)
            (*(data[i]))[j] += l.data[i]->data[k] * r.data[k]->data[j];
}

void Matrix::ProductPhi(const Matrix & M, const Matrix & Phi)
{
   int dim = M.rows;
   Dimension(dim, dim);
   Zero();
   ...
   for(int k = 0; k < dim; k++){
      ...
      if(t != 0.0)
         for(int i = 0; i < dim; i++)
            data[i]->data[k] += M.data[i]->data[k] * t;
   }
   for(int k = 0; k < dim; k++)
      for(int j = k+1; j < dim; j++){
         t = Phi.data[k]->data[j];
         if(t == 0.0) continue;
         for(int i = 0; i < dim; i++) {// computationally intensive
            data[i]->data[j] += M.data[i]->data[k] * t;
            data[i]->data[k] += M.data[i]->data[j] * t;
         }
      }
}

void Matrix::sProduct(const Matrix & l, const Matrix & r)
   {
   if (l.cols != r.rows)
      error("Matrix.Multiply - Attempted to multiply incompatible matrices\n"
            "Matrices        - %s [%d, %d] + %s [%d, %d]\n",
   ...
   for(int i = 0; i < count; i++)
      for(int j = i; j < count; j++){
         data[i]->data[j] += l.data[i]->data[i] * r.data[i]->data[j];
         for(int k = i+1; k <= j; k++){
            data[i]->data[j] += l.data[i]->data[k] * r.data[k]->data[j];
            data[k]->data[j] += l.data[i]->data[k] * r.data[i]->data[j];
         }
...
void Matrix::sProductPhi(const Matrix & M, const Matrix & Phi)
{
   int dim = M.rows;
   Dimension(dim, dim);
   Zero();
   ...
   for(int j = 0; j < dim; j++){
      t = Phi.data[j]->data[j];
      for(int i = j; i < dim; i++)
         data[j]->data[i] = M.data[i]->data[j] * t;
   }
   for(int k = 0; k < dim; k++)
      for(int j = k+1; j < dim; j++){
         t = Phi.data[k]->data[j];
         if(t == 0.0) continue;
         for(int i = 0; i < k+1; i++){  // computationally intensive
            data[i]->data[j] += M.data[i]->data[k] * t;
            data[i]->data[k] += M.data[i]->data[j] * t;
         }
         for(int i = k+1; i <= j; i++) // computationally intensive
            data[i]->data[j] += M.data[i]->data[k] * t;
      }
   for(int i = 0; i < dim; i++)
...
void Matrix::CholeskyInvert(Matrix & M)
{
   int N = M.rows;
   if (M.cols != M.rows)
      error("Matrix.Inverse - Attempted to invert a non-square matrix\n");
   Dimension(N, N);
   Zero();
   double det;
   if(N==1) (*(data[0]))[0] = 1/M[0][0];
   else if(N==2){
      det = M[0][0]*M[1][1] - M[0][1]*M[0][1];
      (*(data[0]))[0] = M[1][1] / det;
      (*(data[1]))[1] = M[0][0] / det;
      (*(data[0]))[1] = (*(data[1]))[0] = -M[0][1] / det;
   }else if(N==3){
      det = M[0][0] * M[1][1] * M[2][2]
         ...
         - M[0][0] * M[1][2] * M[1][2]
         - M[2][2] * M[0][1] * M[0][1];
      data[0]->data[0] = (M[1][1]*M[2][2] - M[1][2]*M[1][2]) / det;
      (*(data[1]))[1] = (M[0][0]*M[2][2] - M[0][2]*M[0][2]) / det;
      (*(data[2]))[2] = (M[0][0]*M[1][1] - M[0][1]*M[0][1]) / det;
      (*(data[1]))[0] = (*(data[0]))[1] = (M[0][2]*M[1][2] - M[0][1]*M[2][2]) / det;
      (*(data[2]))[0] = (*(data[0]))[2] = (M[0][1]*M[1][2] - M[0][2]*M[1][1]) / det;
      (*(data[2]))[1] = (*(data[1]))[2] = (M[0][1]*M[0][2] - M[0][0]*M[1][2]) / det;
   }else if(N > 3){
      static Cholesky chol;
      chol.Decompose(M);
      chol.Invert();
      Copy(chol.inv);
   }
...
void Matrix::Block2Invert(Matrix & M)
{
   int size = M.rows;
   if(size < 14) {CholeskyInvert(M); return;}
   int count = size / 2;
   Dimension(size, size);
   Zero();

   Matrix Det(count, count);
   Det.Zero();
   for(int k = 0; k < count; k++)
      for(int j = 0; j < count; j++){
         if(M.data[count+k]->data[count+j]==0.0 && M.data[j]->data[count+k]==0.0)
            continue;
         for(int i = 0; i <= j ; i++)
            Det.data[i]->data[j] += M.data[i]->data[k]*M.data[count+k]->data[count+j]
               - M.data[i]->data[count+k]*M.data[j]->data[count+k];
      }

   for(int i = 0; i < count; i++)
      for(int j = i+1; j < count; j++)
         Det.data[j]->data[i] = Det.data[i]->data[j];

   Matrix DetInv;
//   printf("<");
   DetInv.CholeskyInvert(Det);
//   printf(">");
   for(int i = 0; i < count; i++)
      for(int k = 0; k < count; k++){
         double t = M.data[count+i]->data[count+k];
         if(t == 0.0) continue;
         for(int j = i; j < count; j++)
            data[i]->data[j] += t * DetInv.data[k]->data[j];
      }
   for(int i = 0; i < count; i++)
      for(int k = 0; k < count; k++){
         double t = M.data[i]->data[k];
         if(t == 0.0) continue;
         for(int j = i; j < count; j++)
            data[count+i]->data[count+j] += t * DetInv.data[k]->data[j];
      }
   for(int i = 0; i < count; i++)
      for(int k = 0; k < count; k++){
         double t = M.data[i]->data[count+k];
         if(t == 0.0) continue;
         for(int j = i; j < count; j++)
            data[i]->data[count+j] -= t * DetInv.data[k]->data[j];
      }
   for(int i = 0; i < count; i++)
      for(int j = i+1; j < count; j++)
         data[j]->data[count+i] = data[i]->data[count+j];
   for(int i = 0; i < size; i++)
      for(int j = i+1; j < size; j++)
         data[j]->data[i] = data[i]->data[j];
}

void Matrix::Block2Invert(Matrix & M, int extra)
{
   int size = M.rows;
   if(size < 18) {CholeskyInvert(M); return;}
   Dimension(size, size);
   Zero();
   int count = size - extra;
   Matrix AI, E, EI, F(count, count);
   for(int i = 0; i < count; i++)
      for(int j = 0; j < count; j++)
         F.data[i]->data[j] = M.data[i]->data[j];
   AI.Block2Invert(F);
   F.Dimension(count, extra);
   F.Zero();
   for(int k = 0; k < count; k++)
      for(int j = 0; j < extra; j++){
         double t = M.data[k]->data[count+j];
         if(t==0.0) continue;
         for(int i = 0; i < count; i++)
            F.data[i]->data[j] += AI.data[i]->data[k] * t;
      }
   E.Dimension(extra, extra);
   for(int i = 0; i < extra; i++)
      for(int j = 0; j < extra; j++)
         E.data[i]->data[j] = M.data[count+i]->data[count+j];
   for(int i = 0; i < extra; i++)
      for(int k = 0; k < count; k++){
         double t = M.data[count+i]->data[k];
         if(t == 0.0) continue;
         for(int j = i; j < extra; j++)
//         for(int j = 0; j < count; j++)
            E.data[i]->data[j] -= t * F.data[k]->data[j];
      }
   for(int i = 0; i < extra; i++)
      for(int j = i+1; j < extra; j++)
         E.data[j]->data[i] = E.data[i]->data[j];
         
   EI.CholeskyInvert(E);
   for(int i = 0; i < extra; i++)
      for(int j = 0; j < extra; j++)
         data[count+i]->data[count+j] = EI.data[i]->data[j];
   for(int i = 0; i < count; i++)
      for(int j = 0; j < extra; j++){
         for(int k = 0; k < extra; k++)
            data[i]->data[count+j] -= F.data[i]->data[k] * EI.data[k]->data[j];
         data[count+j]->data[i] = data[i]->data[count+j];
      }
   for(int i = 0; i < count; i++)
      for(int j = i; j < count; j++){
         data[i]->data[j] = AI.data[i]->data[j];
         for(int k = 0; k < extra; k++)
            data[i]->data[j] -= data[i]->data[count+k] * F.data[j]->data[k];
         data[j]->data[i] = data[i]->data[j];
      }
}

void Matrix::BlockInvert(Matrix & M, int blockCount, int extra)
{
   int size = M.rows;
   if(size < 20) {CholeskyInvert(M); return;}
   if(extra==0){  // balanced
      if(blockCount==1) CholeskyInvert(M);
      else if(blockCount==2) Block2Invert(M);
      else Block2Invert(M, size/blockCount*(blockCount-2));
   }else Block2Invert(M, (size-extra)/blockCount*(blockCount-2)+extra);
}
   ...
   for(int i = 0; i < extra; i++)
      for(int j = i; j < extra; j++){
         E.data[i]->data[j] = M.data[count+i]->data[count+j];
         for(int k = 0; k < count; k++)
            E.data[i]->data[j] -= M.data[count+i]->data[k] * F.data[k]->data[j];
         E.data[j]->data[i] = E.data[i]->data[j];
      }*/
/*
   for(int i = 0; i < count; i++)
...
*/
   ...
   for(int i = 0; i < count; i++)
      for(int j = i; j < count; j++)
         for(int k = 0; k < count; k++){
            data[i]->data[j] += M.data[count+i]->data[count+k] * DetInv.data[k]->data[j];
            data[count+i]->data[count+j] += M.data[i]->data[k] * DetInv.data[k]->data[j];
            data[i]->data[count+j] -= M.data[i]->data[count+k] * DetInv.data[k]->data[j];
         }
*/
